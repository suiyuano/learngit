    ...
    for i in list1:
        if i[0] == "#":
            if i[1] == "#":
                ...
                # print(str_split)
                last_list_item = 'POS'  # 首行输入行头部处理
                for each_eventment in str_split:
                    last_list_item = last_list_item + '\t' + each_eventment + '_' + '1' + '\t' + each_eventment + '_' + '2'  # 首航输入行中间部分处理
                last_list_item = last_list_item + '\n'  # 首行输入行末尾处理
                last_list.append(last_list_item)
        else:
            i = i.strip('\n')
            str_split = i.split('\t')
            last_list_item = str_split[1]  # 主体输入行头部处理
            str_split = str_split[9:]
            # print(str_split)

            for each_eventment in str_split:
                last_list_item = last_list_item + '\t' + each_eventment[0] + '\t' + each_eventment[2]  # 主题输入行中间部分处理
            last_list_item = last_list_item + '\n'
            last_list.append(last_list_item)
